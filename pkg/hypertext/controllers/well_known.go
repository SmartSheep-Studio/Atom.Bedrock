...
func (ctrl *WellKnownController) oauth(c *fiber.Ctx) error {
	return c.JSON(fiber.Map{
		...
		"authorization_endpoint":   fmt.Sprintf("%s/users/openid/connect", viper.GetString("general.base_url")),
		"token_endpoint":           fmt.Sprintf("%s/api/users/openid/exchange", viper.GetString("general.base_url")),
		"end_session_endpoint":     fmt.Sprintf("%s/api/users/sign-out", viper.GetString("general.base_url")),
...
func (ctrl *WellKnownController) openid(c *fiber.Ctx) error {
	return c.JSON(fiber.Map{
		...
		"authorization_endpoint":   fmt.Sprintf("%s/users/openid/connect", viper.GetString("general.base_url")),
		"token_endpoint":           fmt.Sprintf("%s/api/users/openid/exchange", viper.GetString("general.base_url")),
		"userinfo_endpoint":        fmt.Sprintf("%s/api/users", viper.GetString("general.base_url")),
		"end_session_endpoint":     fmt.Sprintf("%s/api/users/sign-out", viper.GetString("general.base_url")),
		"response_types_supported": []string{"code", "token"},
		"grant_types_supported":    []string{"authorization_code", "implicit", "refresh_token", "password"},
		"subject_types_supported":  []string{"public"},
